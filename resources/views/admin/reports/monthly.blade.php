...
<div class="bg-white rounded-xl shadow-sm p-4 md:p-6">
    ...
    <div class="flex flex-col lg:flex-row lg:justify-between lg:items-center gap-4 mb-6">
        ...
        <div class="flex-1">
            <form method="GET" action="{{ route('admin.reports.monthly') }}" class="flex flex-col sm:flex-row gap-2">
                <div class="flex flex-col sm:flex-row items-start sm:items-center gap-2">
                    <label for="month" class="font-semibold whitespace-nowrap text-green-800">Pilih Bulan:</label>
                    <input type="month" name="month" id="month" value="{{ $selectedMonth }}" 
                           class="border border-gray-300 rounded px-3 py-2 focus:ring-2 focus:ring-green-700 focus:border-transparent">
                    <button type="submit" class="bg-green-800 text-white px-4 py-2 rounded hover:bg-green-900 transition whitespace-nowrap">
                        Tampilkan
                    </button>
                </div>
            </form>
        ...
        <div class="flex flex-col sm:flex-row items-start sm:items-center gap-4">
            <p class="order-2 sm:order-1">
                <a href="{{ route('admin.reports.index') }}" class="text-red-700 hover:underline">
                    &larr; Kembali ke menu laporan
                </a>
            </p>
            <button id="exportExcel" class="order-1 sm:order-2 px-4 py-2 bg-green-800 text-white rounded-lg hover:bg-green-900 transition inline-flex items-center">
    ...
    @if($selectedMonth)
        <div class="mb-6">
            <h2 class="text-lg md:text-xl font-semibold text-green-800">
    ...
    <div class="mb-6 p-4 bg-gray-50 rounded-lg border border-gray-200">
        <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
            <div class="bg-white p-4 rounded-lg shadow-sm border-l-4 border-blue-500">
                ...
                <h3 class="text-xl md:text-2xl font-bold text-green-800">{{ $transaksis->count() }}</h3>
            </div>
            <div class="bg-white p-4 rounded-lg shadow-sm border-l-4 border-green-600">
                <p class="text-sm text-gray-500 mb-1">Total Pendapatan</p>
                <h3 class="text-xl md:text-2xl font-bold text-green-800">Rp {{ number_format($total, 0, ',', '.') }}</h3>
            </div>
            <div class="bg-white p-4 rounded-lg shadow-sm border-l-4 border-purple-500">
                <p class="text-sm text-gray-500 mb-1">Rata-rata per Transaksi</p>
                <h3 class="text-xl md:text-2xl font-bold text-green-800">
                ...
                <table id="transactionTable" class="min-w-full divide-y divide-gray-300">
                                       ...
                                       class="inline-flex items-center px-2 md:px-3 py-1 md:py-2 text-xs md:text-sm bg-yellow-600 hover:bg-yellow-700 text-white rounded-lg transition-colors shadow-sm">
                                        <svg class="w-3 h-3 md:w-4 md:h-4 mr-1" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" 
                                                  d="M9 12h6m-6 4h6m2 5H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z"/>
                                        </svg>
                    ...
                    <tfoot class="bg-gray-50">
                        <tr class="border-t-2 border-green-800">
                            ...
                            <td colspan="3" class="px-3 md:px-6 py-4 md:py-6 font-bold text-lg md:text-xl text-green-800">
