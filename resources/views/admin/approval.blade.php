...
<div class="container mx-auto px-4 py-6">
    <div class="max-w-4xl mx-auto">
        <!-- Header -->
        <div class="bg-yellow-50 rounded-lg shadow-sm border border-yellow-200 p-6 mb-6">
            <h2 class="text-2xl font-semibold text-green-800 mb-2">Permintaan Persetujuan User</h2>
            <p class="text-red-600">Kelola permintaan persetujuan user yang baru mendaftar</p>
        </div>
        ...
        <div class="space-y-4">
            @forelse ($pendingUsers as $user)
                <div class="bg-yellow-50 rounded-lg shadow-sm border border-yellow-200 hover:shadow-md transition-shadow duration-200">
                    <div class="p-6">
                        <div class="flex flex-col lg:flex-row lg:items-center lg:justify-between gap-4">
                            ...
                            <div class="flex-1">
                                <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
                                    <div>
                                        <label class="text-sm font-medium text-red-600 uppercase tracking-wide">Nama</label>
                                        <p class="mt-1 text-green-800 font-medium">{{ $user->name }}</p>
                                    </div>
                                    <div>
                                        <label class="text-sm font-medium text-red-600 uppercase tracking-wide">Email</label>
                                        <p class="mt-1 text-green-800">{{ $user->email }}</p>
                                    </div>
                                    <div>
                                        <label class="text-sm font-medium text-red-600 uppercase tracking-wide">Role</label>
            ...
            @empty
                <!-- Empty State -->
                <div class="bg-yellow-50 rounded-lg shadow-sm border border-yellow-200 p-12 text-center">
                    ...
                    <p class="text-red-600">Saat ini tidak ada user yang menunggu persetujuan.</p>
                </div>
            @endforelse
        </div>
    </div>
